...
                    {{-- 🔑 Aseguramos que la fecha existe antes de formatear --}}
...
                                        @elseif($item->item_type === 'kit' && $item->kit)
                                            <i class="fas fa-cubes text-info"></i> 
                                            <strong>{{ $item->kit->name }} (Kit)</strong>
                                            <small class="d-block text-muted">Contiene {{ $item->kit->components->count() ?? 0 }} productos</small>
                                        @else
                                            Ítem Eliminado
                                        @endif
                                    </td>
...
                                            {{ $item->product->unit->abbreviation ?? 'unid' }}
                                        @else
                                            unid.
                                        @endif
                                    </td>
                                    <td>
                                        {{-- El stock solo aplica al producto simple --}}
                                        @if($item->item_type === 'product')
                                            <span class="badge badge-{{ ($item->product->stock ?? 0) < $item->quantity_requested ? 'danger' : 'success' }}">
                                                {{ $item->product->stock ?? 0 }}
                                            </span>
                                        @else
                                            <span class="badge badge-info">Ver componentes</span>
                                        @endif
                                    </td>
                                    <td>${{ number_format($item->unit_price_at_request, 2) }}</td>
                                </tr>
                                
                                {{-- 🔑 Fila secundaria: Componentes del Kit (NUEVO) --}}
                                @if($item->item_type === 'kit' && $item->kit && $item->kit->components->count())
                                    <tr>
                                        <td colspan="4" class="p-0 border-0">
                                            <table class="table table-sm table-borderless m-0 bg-light">
                                                <tbody>
                                                    @foreach($item->kit->components as $component)
                                                        <tr class="text-muted small">
                                                            <td style="padding-left: 2rem;">
                                                                <i class="fas fa-arrow-right"></i> **Componente:** {{ $component->name }}
                                                            </td>
                                                            <td style="width: 15%" class="font-weight-bold">
                                                                {{ $item->quantity_requested * $component->pivot->quantity_required }}
                                                                {{ $component->unit->abbreviation ?? 'unid' }} (Total)
                                                            </td>
                                                            <td style="width: 15%">
                                                                Stock: {{ $component->stock }}
                                                            </td>
                                                            <td style="width: 15%">
                                                                {{-- Espacio para el costo si es necesario --}}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
