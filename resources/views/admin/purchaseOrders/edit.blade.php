    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        <div>
            <h1 class="text-dark font-weight-bold" style="font-size: 1.75rem;">
                <i class="fas fa-edit mr-2 text-primary"></i> Editar Orden de Compra: {{ $purchaseOrder->code }}
            </h1>
            <p class="text-muted mb-0">Modifique los datos generales y los ítems de esta orden de compra.</p>
        </div>
        <a href="{{ route('admin.purchaseOrders.show', $purchaseOrder) }}" class="btn btn-outline-secondary font-weight-bold btn-custom">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <div class="form-group mb-0">
                                    ...
                                    <input type="text" name="code" class="form-control form-control-custom font-weight-bold" value="{{ $purchaseOrder->code }}" readonly style="background-color: #f3f4f6;">
                                </div>
                            </div>
                            <div class="col-md-5 mb-3">
                                <div class="form-group mb-0">
                                    ...
                                    <div class="input-group">
                                        <select name="supplier_id" id="supplier_id" class="form-control form-control-custom select2" required>
                            ...
                            <div class="col-md-3 mb-3">
                                <div class="form-group mb-0">
                                    ...
                                    <input type="date" name="date_issued" class="form-control form-control-custom" value="{{ old('date_issued', $purchaseOrder->date_issued->format('Y-m-d')) }}" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row border-top pt-3 mt-1">
                            <div class="col-md-4 mb-3">
                                <div class="form-group mb-0">
                                    ...
                                    <input type="date" name="delivery_date" class="form-control form-control-custom" value="{{ old('delivery_date', $purchaseOrder->delivery_date?->format('Y-m-d')) }}">
                                </div>
                            </div>
                            <div class="col-md-8 mb-3">
                                <div class="form-group mb-0">
                                    <label for="delivery_address" class="text-xs font-weight-bold text-secondary mb-1">Dirección de Entrega</label>
                                    <input type="text" name="delivery_address" class="form-control form-control-custom" value="{{ old('delivery_address', $purchaseOrder->delivery_address) }}" placeholder="Dirección donde recibir la mercancía">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="table mb-0" id="itemsTable">
                                ...
                                <tbody id="itemsBody">
                                    @foreach($purchaseOrder->items as $index => $item)
                                        <tr>
                                            <td>
                                                <select name="items[{{ $index }}][item_type]" class="form-control form-control-custom type-selector" style="height: 38px !important;" onchange="toggleType(this)">
                                            ...
                                            <td>
                                                <select name="items[{{ $index }}][product_id]" class="form-control select2-product" {{ $item->item_type === 'product' ? 'required' : 'style=display:none' }}>
                                                    <option value="">Seleccione producto...</option>
                                                    @foreach($products as $product)
                                                        <option value="{{ $product->id }}" {{ $item->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }} ({{ $product->code ?? 'S/C' }})</option>
                                                    @endforeach
                                                </select>
                                                <select name="items[{{ $index }}][kit_id]" class="form-control select2-kit" {{ $item->item_type === 'kit' ? 'required' : 'style=display:none' }}>
                                            ...
                                            <td>
                                                <input type="number" name="items[{{ $index }}][quantity]" class="form-control form-control-custom item-qty" min="1" value="{{ old("items.$index.quantity", $item->quantity) }}" required>
                                            </td>
                                            <td>
                                                <input type="number" step="0.01" name="items[{{ $index }}][unit_cost]" class="form-control form-control-custom item-cost" min="0" value="{{ old("items.$index.unit_cost", $item->unit_cost) }}" required>
                                            </td>
                                            ...
                                            <td class="text-center align-middle">
                                                ...
                                                <input type="checkbox" name="items[{{ $index }}][is_exempt]" value="1"
                                                       class="row-iva-switch"
                                                       {{ old("items.$index.is_exempt", $item->is_exempt ?? false) ? 'checked' : '' }}
                                                       style="width: 20px; height: 20px; cursor: pointer;">
                                            </td>
                                            <td class="text-center align-middle">
                                                <button type="button" class="btn btn-sm btn-outline-danger remove-item" {{ $purchaseOrder->items->count() <= 1 ? 'style=display:none' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <div class="form-group mb-0">
                                    ...
                                    <textarea name="terms" id="terms" rows="3" class="form-control form-control-custom" placeholder="Condiciones de pago, garantías, tiempos de envío..." style="height: auto !important;">{{ old('terms', $purchaseOrder->terms) }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-6 mb-3">
                                <div class="form-group mb-0">
                                    <label for="notes" class="text-xs font-weight-bold text-secondary mb-1">Notas Internas</label>
                                    <textarea name="notes" id="notes" rows="3" class="form-control form-control-custom" placeholder="Comentarios internos (no se imprimen en el PDF)..." style="height: auto !important;">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-custom bg-white">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group mb-3" id="exchangeRateGroup">
                                ...
                                <input type="number" step="0.0001" name="exchange_rate" id="exchangeRate" class="form-control form-control-custom font-weight-bold" value="{{ old('exchange_rate', $purchaseOrder->exchange_rate) }}">
                            </div>

                            <div class="form-group mb-3">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" id="iva_exempt" name="iva_exempt" value="1" {{ $purchaseOrder->iva_exempt ? 'checked' : '' }}>
                    ...
                    <div class="card card-custom">
                        <div class="card-body d-flex flex-column p-3">
                            <button type="button" class="btn btn-primary btn-block btn-custom font-weight-bold mb-2 shadow-sm" id="saveOrderBtn">
                                <i class="fas fa-save mr-1"></i> Actualizar Orden de Compra
                            </button>
                            <a href="{{ route('admin.purchaseOrders.show', $purchaseOrder) }}" class="btn btn-block btn-outline-secondary font-weight-bold btn-custom">
        ...
        $(document).ready(function() {
            ...
            document.getElementById('saveOrderBtn').addEventListener('click', function() {
                confirmAction({
                    ...
                    message: '¿Está seguro de actualizar esta orden de compra?',
                    alert: 'Verifique que todos los datos y productos sean correctos.',
