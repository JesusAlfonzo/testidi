        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const newOption = new Option(
                    ...
                    response.supplier.id, false, false
                );
                $('#supplier_id').append(newOption).val(response.supplier.id).trigger('change');
                Swal.fire({ icon: 'success', title: '¡Éxito!', text: response.message, timer: 2000, showConfirmButton: false });
            },
            error: function(xhr) {
                if (xhr.status === 422) {
                    Swal.fire({ icon: 'error', title: 'Error de validación', text: 'Verifique los campos del proveedor.' });
                } else {
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Hubo un error al guardar el proveedor.' });
                }
            },
            complete: function() {
